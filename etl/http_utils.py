...
class RecursionSafeSession:
    """HTTP session with recursion depth protection and robust error handling."""
    ...
    def safe_get(self, url: str, timeout: int = DEFAULT_TIMEOUT, **kwargs) -> Optional[requests.Response]:
        ...
        try:
            ...
            if 'original_limit' in locals() and original_limit < DEFAULT_RECURSION_LIMIT:
...
def download_with_retries(url: str, output_path: Path, 
                         max_retries: int = 3, timeout: int = DEFAULT_TIMEOUT) -> bool:
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if total_size > MAX_RESPONSE_SIZE_MB * 1024 * 1024:
                            log.warning(f"[DOWNLOAD] File too large, stopping download")
...
def validate_response_content(response: requests.Response) -> bool:
    ...
    try:
        ...
        if any(t in content_type for t in ['json', 'xml']) and len(response.content) > MAX_RESPONSE_SIZE_MB * 1024 * 1024:
